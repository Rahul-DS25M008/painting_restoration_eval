"""Isolated official-HINT batch worker for Notebook 37."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys
from time import perf_counter
from typing import Any, Mapping

import numpy as np
from PIL import Image

from restoration_eval.hint_mat_selection import (
    CANDIDATE_COLUMNS,
    atomic_write_csv,
    atomic_write_json,
    calculate_file_sha256,
    exact_mask_composite,
    load_resumable_candidate_records,
    prepare_model_inputs,
    utc_now_iso,
    validate_restored_output,
)


WORKER_VERSION = "1.0.2"


def _load_job(path: Path) -> dict[str, Any]:
    with path.open("r", encoding="utf-8-sig") as handle:
        job = json.load(handle)
    if job.get("job_schema_version") != "hint_mat_worker_job.v1":
        raise ValueError("Unsupported HINT worker job schema")
    if job.get("model_key") != "hint":
        raise ValueError("The HINT worker received a non-HINT job")
    return job


def _load_generator(repo: Path, checkpoint: Path, device: Any) -> tuple[Any, dict[str, Any]]:
    if str(repo) not in sys.path:
        sys.path.insert(0, str(repo))
    import torch
    from src.networks import HINT

    started = perf_counter()
    generator = HINT().to(device).eval().requires_grad_(False)
    try:
        payload = torch.load(checkpoint, map_location=device, weights_only=False)
    except TypeError:
        payload = torch.load(checkpoint, map_location=device)
    state = payload.get("generator", payload) if isinstance(payload, Mapping) else payload
    if not isinstance(state, Mapping):
        raise TypeError("The HINT checkpoint does not contain a generator state mapping")
    normalized = {
        (str(key)[7:] if str(key).startswith("module.") else str(key)): value
        for key, value in state.items()
    }
    incompatibility = generator.load_state_dict(normalized, strict=False)
    model_state_key_count = len(generator.state_dict())
    matched = model_state_key_count - len(incompatibility.missing_keys)
    if matched <= 0:
        raise ValueError("No HINT generator weights matched the released architecture")
    return generator, {
        "model_load_seconds": round(perf_counter() - started, 3),
        "matched_state_keys": matched,
        "model_state_key_count": model_state_key_count,
        "missing_state_keys": len(incompatibility.missing_keys),
        "unexpected_state_keys": len(incompatibility.unexpected_keys),
        "all_required_state_keys_loaded": len(incompatibility.missing_keys) == 0,
        "required_state_coverage": round(matched / model_state_key_count, 6),
        "loading_policy": "official_strict_false",
    }


def _infer(generator: Any, row: Mapping[str, Any], size: int, device: Any) -> np.ndarray:
    import torch
    import torch.nn.functional as torch_f

    image, missing_small, _, _ = prepare_model_inputs(
        row["input_image_path"], row["mask_path"], inference_size=size
    )
    image_tensor = (
        torch.from_numpy(image.transpose(2, 0, 1).copy()).float().unsqueeze(0).to(device) / 255.0
    )
    mask_tensor = torch.from_numpy(missing_small.copy()).float().unsqueeze(0).unsqueeze(0).to(device)
    masked_input = image_tensor * (1.0 - mask_tensor) + mask_tensor
    mask_half = torch_f.interpolate(mask_tensor, scale_factor=0.5, mode="nearest")
    mask_quarter = torch_f.interpolate(mask_tensor, scale_factor=0.25, mode="nearest")
    mask_tiny = torch_f.interpolate(mask_tensor, scale_factor=0.125, mode="nearest")
    with torch.inference_mode():
        output = generator(masked_input, mask_tensor, mask_half, mask_quarter, mask_tiny)
    return (
        output[0].detach().clamp(0, 1).mul(255).round().byte().permute(1, 2, 0).cpu().numpy()
    )


def execute_job(job: Mapping[str, Any]) -> dict[str, Any]:
    import pandas as pd
    import torch

    repo = Path(str(job["repository_path"]))
    checkpoint = Path(str(job["checkpoint_path"]))
    if not repo.is_dir() or not checkpoint.is_file():
        raise FileNotFoundError("HINT repository or checkpoint is unavailable")
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is unavailable and CPU fallback is not authorized")
    torch.manual_seed(int(job["model"]["seed"]))
    torch.cuda.manual_seed_all(int(job["model"]["seed"]))
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    device = torch.device("cuda")
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()
    generator, load_audit = _load_generator(repo, checkpoint, device)

    rows = [dict(item) for item in job["candidates"]]
    records_by_id = load_resumable_candidate_records(job)
    native_size = int(job["model"]["native_probe_size"])
    fallback_size = int(job["model"]["fallback_size"])
    active_size = native_size
    fallback_reason = ""
    started_all = perf_counter()
    total = len(rows)
    executed_count = 0
    for number, row in enumerate(rows, start=1):
        candidate_id = str(row["candidate_id"])
        if candidate_id in records_by_id:
            resumed = records_by_id[candidate_id]
            print(
                f"HINT progress {number}/{total}: {row['case_id']} -> "
                f"resumed_{resumed.get('status', 'unknown')}",
                flush=True,
            )
            continue
        executed_count += 1
        row["started_at_utc"] = utc_now_iso()
        row["actual_device"] = "cuda"
        row["model_load_seconds"] = (
            load_audit["model_load_seconds"] if executed_count == 1 else 0.0
        )
        case_started = perf_counter()
        try:
            inference_started = perf_counter()
            try:
                generated = _infer(generator, row, active_size, device)
            except RuntimeError as exc:
                if number == 1 and active_size == native_size:
                    reason = "cuda_out_of_memory" if "out of memory" in str(exc).lower() else type(exc).__name__
                    fallback_reason = f"native_768_{reason}"
                    torch.cuda.empty_cache()
                    active_size = fallback_size
                    generated = _infer(generator, row, active_size, device)
                else:
                    raise
            row["inference_seconds"] = round(perf_counter() - inference_started, 3)
            _, _, missing_768, source_768 = prepare_model_inputs(
                row["input_image_path"], row["mask_path"], inference_size=active_size
            )
            composite = exact_mask_composite(generated, source_768, missing_768)
            output_path = Path(str(row["restored_path"]))
            output_path.parent.mkdir(parents=True, exist_ok=True)
            Image.fromarray(composite, mode="RGB").save(output_path, format="PNG", compress_level=6)
            technical = validate_restored_output(
                output_path, row["input_image_path"], row["mask_path"]
            )
            row.update(technical)
            row["restored_sha256"] = calculate_file_sha256(output_path)
            row["inference_width"] = active_size
            row["inference_height"] = active_size
            row["output_width"] = int(composite.shape[1])
            row["output_height"] = int(composite.shape[0])
            row["runtime_seconds"] = round(perf_counter() - case_started, 3)
            row["gpu_peak_memory_bytes"] = int(torch.cuda.max_memory_allocated())
            row["generator_name"] = type(generator).__name__
            row["status"] = "completed" if technical["technical_validation_passed"] else "failed"
            row["failure_type"] = "none" if row["status"] == "completed" else "technical_validation"
            row["issue"] = fallback_reason
        except Exception as exc:
            row.update(
                {
                    "runtime_seconds": round(perf_counter() - case_started, 3),
                    "status": "failed",
                    "failure_type": "cuda_out_of_memory" if "out of memory" in str(exc).lower() else "inference_failure",
                    "error_type": type(exc).__name__,
                    "error_message": str(exc),
                    "issue": fallback_reason,
                }
            )
        row["completed_at_utc"] = utc_now_iso()
        row["generator_version"] = WORKER_VERSION
        records_by_id[candidate_id] = row
        ordered_records = [
            records_by_id[str(item["candidate_id"])]
            for item in rows
            if str(item["candidate_id"]) in records_by_id
        ]
        frame = pd.DataFrame(ordered_records).reindex(columns=CANDIDATE_COLUMNS)
        atomic_write_csv(frame, job["checkpoint_path_csv"])
        atomic_write_json(
            job["progress_path"],
            {
                "model_key": "hint", "completed_cases": len(records_by_id), "total_cases": total,
                "successful_cases": sum(
                    item.get("status") == "completed"
                    for item in records_by_id.values()
                ),
                "active_inference_size": active_size, "updated_at_utc": utc_now_iso(),
            },
        )
        print(f"HINT progress {number}/{total}: {row['case_id']} -> {row['status']}", flush=True)
        if row["status"] != "completed" and bool(job["execution"]["stop_after_model_failure"]):
            break
        if float(row["runtime_seconds"] or 0) > float(job["execution"]["per_case_timeout_seconds"]):
            break

    payload = {
        "result_schema_version": "hint_mat_worker_result.v1",
        "worker_version": WORKER_VERSION,
        "model_key": "hint",
        "model_load_audit": load_audit,
        "native_probe_size": native_size,
        "actual_inference_size": active_size,
        "fallback_reason": fallback_reason,
        "runtime_seconds": round(perf_counter() - started_all, 3),
        "completed_rows": len(records_by_id),
        "successful_rows": sum(
            item.get("status") == "completed"
            for item in records_by_id.values()
        ),
        "resumed_rows": len(records_by_id) - executed_count,
        "status": "completed" if len(records_by_id) == total and all(
            item.get("status") == "completed"
            for item in records_by_id.values()
        ) else "failed",
        "completed_at_utc": utc_now_iso(),
    }
    atomic_write_json(job["result_path"], payload)
    return payload


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--job", required=True)
    args = parser.parse_args()
    try:
        result = execute_job(_load_job(Path(args.job)))
        print(json.dumps(result, ensure_ascii=False), flush=True)
        return 0 if result["status"] == "completed" else 1
    except Exception as exc:
        print(f"HINT worker failed: {type(exc).__name__}: {exc}", file=sys.stderr, flush=True)
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
