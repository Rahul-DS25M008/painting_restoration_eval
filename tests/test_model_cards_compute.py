...
class ModelCardsComputeContractTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.candidates = {
            ...
            "hint_places2": pd.read_csv(
                cls.inputs["hint_candidates_path"], low_memory=False
            ),
        ...
        cls.runtimes = {
            ...
            "hint_places2": pd.read_csv(cls.inputs["hint_runtime_path"]),
            "stable_diffusion_inpainting": pd.read_csv(
        ...
        cls.manifests["12a"] = json.loads(
            cls.inputs["manifest_12a_path"].read_text(encoding="utf-8-sig")
        )
    ...
    def test_config_identity_and_arithmetic(self) -> None:
        ...
        self.assertEqual(expected["model_card_rows"], 5)
        self.assertEqual(expected["observed_compute_rows"], 32)
        self.assertEqual(expected["projection_rows"], 10)
        self.assertEqual(expected["compute_rows"], 42)
    ...
    def test_compute_table_retains_observed_and_projected_distinction(self) -> None:
        ...
        self.assertEqual(len(self.projections), 10)
        self.assertEqual(len(self.compute), 42)
        checks = validate_compute_scalability(self.compute, config=self.config)
        self.assertTrue(checks["passed"].all(), checks.to_string(index=False))
        sdxl_full = self.projections.loc[
            self.projections["model_id"].eq("sdxl_inpainting")
            & self.projections["scenario_id"].eq("projected_300_sdxl_full_design")
        ].iloc[0]
        self.assertEqual(sdxl_full["status"], "ok")
        self.assertEqual(int(sdxl_full["case_count"]), 2620)
        self.assertEqual(int(sdxl_full["candidate_count"]), 2620)
        scaled = self.projections.loc[
            self.projections["scenario_id"].eq("projected_600_current_design_mix")
        ]
        self.assertEqual(len(scaled), 5)
        current_sd = scaled.loc[
            scaled["model_id"].eq("stable_diffusion_inpainting")
        ].iloc[0]
        self.assertEqual(int(current_sd["case_count"]), 5240)
        self.assertEqual(int(current_sd["candidate_count"]), 17040)
        self.assertAlmostEqual(float(current_sd["candidate_multiplier"]), 8520 / 2620)
    ...
    def test_quality_evidence_is_complete_and_not_a_combined_score(self) -> None:
        self.assertEqual(len(self.quality_values), 99)
        self.assertEqual(len(self.quality_wins), 9)
    ...
    def test_all_five_markdown_cards_follow_locked_structure(self) -> None:
