<!-- Approved README mock, saved 2026-09-04 at the user's request.
Do not replace the root README until the dashboard discussion is resolved.
Preserve this approved structure and wording; revise only agreed changes.
The figure uses ../outputs/ so this draft renders from docs/. When promoted
to the repository-root README, change that prefix to outputs/.
Deployment status must be verified before changing its status sentence.
-->
...
[Explore the dashboard](https://fhtw-painting-restoration.streamlit.app/) · [Full evaluation report](https://github.com/Rahul-DS25M008/painting_restoration_eval/blob/main/outputs/33_final_evaluation_report/reports/final_evaluation.html) · [Browse case and painting reports](https://github.com/Rahul-DS25M008/painting_restoration_eval/blob/main/outputs/32_case_and_painting_report_generation/reports/index.html)

**Status:** The 36-notebook computational pipeline is complete. Public dashboard deployment is being verified.
...
Use it to compare restorations, inspect regions and heatmaps, filter candidates, and download reports. It reads existing evidence; it does not run restoration models or recompute scientific metrics.
